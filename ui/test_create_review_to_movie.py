...
class TestMoviePage:
    ...
    def test_create_review_movie(self, authorized_page):
        with allure.step("Генерация рандомных данных"):
            fake = Faker('ru_RU')
            ...
            allure.attach(f"Фильм #{random_movie_index}, рейтинг: {random_rating}, отзыв: '{random_review}'", "Сгенерированные данные", allure.attachment_type.TEXT)

        with allure.step(f"Создание отзыва {random_review} у фильма с индексом: {random_movie_index}"):
        ...
        with allure.step("Проверка, что текст отзыва совпадает с тем что мы нагенерировали"):
        ...
        with allure.step("Проверка всплывающего окна об успешном добавлении отзыва"):
